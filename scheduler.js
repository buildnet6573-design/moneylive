 ...
 * MONEYLIVE 스케줄러 v3
 * 핵심 변경:
 * - API 빈값/실패 시 이전 data.json 값 유지 (덮어쓰지 않음)
 * - 수급/뎁스/프로그램: 0값이면 이전 데이터 유지
 * - Yahoo 날짜 한국시간 변환 수정 (1/1 목 버그 수정)
 * - KORZ 등 유동성 낮은 종목 안전 처리
 */
...
async function fetchIndex(token, prev) {
  ...
  const fetchOne = async (iscd, name, prevVal) => {
    try {
      const r = await kisGet('/uapi/domestic-stock/v1/quotations/inquire-daily-indexchartprice', token, 'FHKUP03500100',
      ...
      return {
        ...
        change: prev2 ? parseFloat(((parseFloat(last.bstp_nmix_prpr)-parseFloat(prev2.bstp_nmix_prpr))/parseFloat(prev2.bstp_nmix_prpr)*100).toFixed(2)) : parseFloat(last.bstp_nmix_ctrt||0),
        history: hist.slice(-5).map((h,i,arr) => ({
          date: parseKisDate(h.stck_bsdt),
          close: h.bstp_nmix_prpr,
          change: i>0 ? parseFloat(((parseFloat(h.bstp_nmix_prpr)-parseFloat(arr[i-1].bstp_nmix_prpr))/parseFloat(arr[i-1].bstp_nmix_prpr)*100).toFixed(2)) : parseFloat(h.bstp_nmix_ctrt||0)
        }))
      };
    } catch(e) { log(`  ⚠️ ${name} 실패→이전유지: ${e.message}`); return prevVal||null; }
  };
...
async function fetchSupply(token, prev) {
  log('💰 수급 동향 수집 중...');
  try {
    const r = await kisGet('/uapi/domestic-stock/v1/quotations/inquire-investor', token, 'FHKST01010900',
    ...
    // ★ 전부 0이면 장 마감 후 → 이전 데이터 유지
    if (f===0 && inst===0 && ind===0 && prev.supply) {
      log('  ⚠️ 수급 모두 0 (장 마감) → 이전 데이터 유지');
      return prev.supply;
    }
  ...
  } catch(e) { log(`  ⚠️ 수급 실패→이전유지: ${e.message}`); return prev.supply||null; }
}

async function fetchProgram(token, prev) {
  log('🤖 프로그램 매매 수집 중...');
  try {
    const r = await kisGet('/uapi/domestic-stock/v1/quotations/inquire-program-trade-by-stock', token, 'FHPPG04650100',
      { FID_COND_MRKT_DIV_CODE:'J', FID_INPUT_ISCD:'0001', FID_INPUT_DATE_1:getDate(-1) });
    const d = r?.output?.[0];
  ...
  } catch(e) { log(`  ⚠️ 프로그램매매 실패→이전유지: ${e.message}`); return prev.program||null; }
}
...
async function fetchDepth(token, prev) {
  ...
  try {
    const r = await kisGet('/uapi/domestic-stock/v1/quotations/inquire-daily-indexchartprice', token, 'FHKUP03500100',
      { FID_COND_MRKT_DIV_CODE:'U', FID_INPUT_ISCD:'0001', FID_INPUT_DATE_1:getDate(-1), FID_INPUT_DATE_2:getDate(0), FID_PERIOD_DIV_CODE:'D' });
    const rows = r?.output2;
    if (!rows?.length) throw new Error('데이터없음');
    const last = rows[0];
    ...
    if (up+neu+dn===0 && prev.depth) { log('  ⚠️ 뎁스 0 → 이전 데이터 유지'); return prev.depth; }
    log(`  상승:${up} / 보합:${neu} / 하락:${dn}`);
    return { up, neu, dn };
  } catch(e) { log(`  ⚠️ 마켓뎁스 실패→이전유지: ${e.message}`); return prev.depth||null; }
}

async function fetchKrEtf(token, prev) {
  log('🇰🇷 한국 ETF 수집 중...');
  const codes = { kodex200:'069500', kodexkosdaq:'229200', kodexlev:'122630', kodexinv:'114800', kodexinv2:'251340', kodexsp:'379800' };
  const etf = { ...(prev.etf||{}) };
...
async function fetchKrStocks(token, prev) {
  ...
  try {
    const volRes = await kisGet('/uapi/domestic-stock/v1/ranking/trading-value', token, 'FHPST01710000',
      { FID_COND_MRKT_DIV_CODE:'J', FID_COND_SCR_DIV_CODE:'20171', FID_INPUT_ISCD:'0000', FID_DIV_CLS_CODE:'0',
    ...
    const fetchRank = async (blng) => {
      try {
        const r = await kisGet('/uapi/domestic-stock/v1/ranking/investor', token, 'FHPST01720000',
          { FID_COND_MRKT_DIV_CODE:'J', FID_COND_SCR_DIV_CODE:'20172', FID_INPUT_ISCD:'0000', FID_DIV_CLS_CODE:'0',
            FID_BLNG_CLS_CODE:blng, FID_TRGT_CLS_CODE:'111111111', FID_TRGT_EXLS_CLS_CODE:'000000',
            FID_INPUT_PRICE_1:'', FID_INPUT_PRICE_2:'', FID_VOL_CNT:'', FID_INPUT_DATE_1:'' });
        return (r?.output||[]).slice(0,5).map(s => ({ name:s.hts_kor_isnm, code:s.mksc_shrn_iscd, chg:parseFloat(s.prdy_ctrt||0), amt:parseInt(s.ntby_qty||0) }));
      } catch { return null; }
    };
    ...
    return result;
  } catch(e) { log(`  ⚠️ KR종목 실패→이전유지: ${e.message}`); return prevStocks||null; }
}
...
async function collect() {
  ...
  log(`  이전 데이터: ${Object.keys(prev).length>0 ? '로드됨 (실패 시 유지)' : '없음'}`);
...
log('🚀 MONEYLIVE 스케줄러 v3 시작');
log('핵심: API 빈값/실패 시 이전 데이터 유지');
